...
```
### Final Model, concluded from common sense and correlation matrix, horsepower
...
for(i in 1:N){
mpg[i] ~ dnorm(mu[i], tau)
mu[i] <- alpha + beta4*model.year[i] + beta5*horsepower[i] + beta6*acceleration[i] 
}
# priors
alpha ~ dnorm(0, .001)
beta4 ~ dunif(-1000,1000)
...
sims <- bugs(data=data, inits=inits, parameters.to.save = c("alpha", "beta4", "beta5", "beta6", "tau"), model.file = model, n.chains=1, n.iter=100000, n.burnin=10000)
...
for(i in 1:N){
mpg[i] ~ dnorm(mu[i], tau)
mu[i] <- alpha + beta4*model.year[i] + beta5*displacement[i] + beta6*acceleration[i] 
}
# priors
alpha ~ dnorm(0, .001)
beta4 ~ dunif(-1000,1000)
...
### Final Model, concluded from common sense and correlation matrix, normals
